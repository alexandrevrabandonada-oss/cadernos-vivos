...
import { z } from "zod";
import { parseCadernoJson, parseMapaJson, parseDebateJson, parseAcervoJson } from "@/lib/schemas";
...
export async function listCadernos(): Promise<CadernoMeta[]> {
  ...
  for (const slug of slugs) {
    let metaPath = path.join(base, slug, "caderno.json");
try {
  await fs.stat(metaPath);
} catch {
  metaPath = path.join(base, slug, "meta.json");
}
    const raw = await fs.readFile(metaPath, "utf8");
    items.push(CadernoMeta.passthrough().parse(parseCadernoJson(raw)));
  }
...
const acervoPath = path.join(base, slug, "acervo.json");
  ...
  const metaParsed = CadernoMeta.passthrough().parse(metaRaw);
  const meta = { ...metaParsed, slug: metaParsed.slug ?? slug };
