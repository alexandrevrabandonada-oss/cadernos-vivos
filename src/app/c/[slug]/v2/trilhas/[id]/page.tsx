          ...
          {Array.isArray(trail.steps) && trail.steps.length ? (
            <ol style={{ margin: 0, paddingLeft: 18, display: "grid", gap: 8 }}>
              {(Array.isArray(trail.steps) ? (trail.steps as unknown[]) : []).map((st, idx) => {
                ...
                const kind = (typeof stepObj["kind"] === "string" ? String(stepObj["kind"]) : (typeof stepObj["type"] === "string" ? String(stepObj["type"]) : "")) || "";
                return (
                  <li key={(stepId + "-" + String(idx))} style={{ lineHeight: 1.45 }}>
                    ...
                    {kind ? <span style={{ marginLeft: 8, fontSize: 12, opacity: 0.75 }}>({kind})</span> : null}
                  </li>
                );
              })}
            </ol>
