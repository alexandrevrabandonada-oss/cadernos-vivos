...
export default function MapaDockV2(props: { slug: string; mapa?: unknown }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("cv:map:select", onSelect as EventListener);
    return () => window.removeEventListener("cv:map:select", onSelect as EventListener);
  }, []);
  const [copied, setCopied] = useState<string>("");

  useEffect(() => {
    const onHash = () => setHashId(readHashId());
