...
PatchText "src\components\v2\V2Nav.tsx" {
  ...
  for ($i = 0; $i -lt $lines.Length; $i++) {
    ...
    if ($ln -match '\bactive\b' -and $ln -match '"mapa"' -and $ln -notmatch '"provas"') {
      # tenta adicionar depois de "mapa"
...
PatchText "src\components\v2\HomeV2Hub.tsx" {
  ...
  # insere antes do link de "Voltar" (href={"/c/" + slug})
...
$rep.Add("# CV — Step D3 — ProvasV2 (v0_103)") | Out-Null
$rep.Add("") | Out-Null
...
$rp = WriteReport "cv-step-d3-provasv2-v0_103.md" ($rep -join "`n")
