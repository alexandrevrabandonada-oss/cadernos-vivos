...
# fallbacks (caso algo não exista no bootstrap)
...
# PATCH: markdown.ts (renderer minimalista, sem escapes treteiros)
$ts = @(
...
# Corrige aspas simples duplicadas no retorno do link (pra evitar escapes)
$ts = $ts.Replace("href='' + allowed + ''", "href='" + '" + allowed + "' + "'" )
$ts = $ts.Replace("target=''_blank''", "target='" + '" + "_blank" + "' + "'" )
$ts = $ts.Replace("rel=''noreferrer''", "rel='" + '" + "noreferrer" + "' + "'" )

WriteUtf8NoBom $mdPath $ts
WL "[OK] patched: src/lib/markdown.ts (stable)"

$now = Get-Date -Format "yyyy-MM-dd HH:mm"
$rep = @(
  ("# CV Hotfix - Markdown stable v0.22e - " + $now),
  "",
  "## O que fez",
...
$repPath = NewReport "cv-hotfix-markdown-stable-v0_22e.md" $rep
...
WL "[OK] Hotfix aplicado: markdown.ts parse OK."