...
# paths
$target = Join-Path $root 'src\app\c\[slug]\v2\provas\page.tsx'
...
$ts = Get-Date -Format 'yyyyMMdd-HHmmss'
$reportPath = Join-Path $reportsDir ('cv-step-b4e-refactor-v2-provas-motor-' + $ts + '.md')
...
$rep.Add('# CV — Step B4e: Refactor V2 Provas to use safe motor')
$rep.Add('')
$rep.Add('- when: ' + $ts)
...
$rep.Add('')
$rep.Add('## NEXT')
if ($verifyExit -eq 0) { $rep.Add('- OK. Próximo: B4f (Trilhas V2).') } else { $rep.Add('- Verify falhou. Corrigir e re-rodar.') }
