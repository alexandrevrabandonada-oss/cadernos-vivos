# CV — V2 Tijolo Extra — Crosslinks + Lint Polish — v0_27b
...
function TextHas([string[]]$lines, [string]$needle) {
...
if (Test-Path -LiteralPath $headerPath) {
  ...
  } elseif (-not (TextHas $lines "slug")) {
    Write-Host "[SKIP] CadernoHeader não parece ter slug em escopo — não vou inserir link V2 aqui."
  } else {
...
if (Test-Path -LiteralPath $v2NavPath) {
  ...
  if (TextHas $lines 'label: "V1"' -or TextHas $lines "label: 'V1'") {
...
# ---------- PATCH 4) Trilhas page: trocar aspas " no texto JSX por &quot; ----------
...
# ---------- REPORT (sem backticks pra não quebrar o parser) ----------
$report = @"
# CV — V2 Tijolo Extra v0_27b — Crosslinks + Lint Polish

## O que mudou
- V1 NavPills (CadernoHeader): adiciona link para /v2 (beta) quando existir slug em escopo.
- V2Nav: adiciona link para V1.
- TimelineV2: remove window.location.hash = hash; e usa scrollIntoView.
- V2 Trilhas page: converte aspas duplas em texto JSX para &quot; (evita react/no-unescaped-entities).
...
WriteReport "cv-v2-tijolo-extra-crosslinks-lintpolish-v0_27b.md" $report | Out-Null
Write-Host "[OK] v0_27b aplicado e verificado."