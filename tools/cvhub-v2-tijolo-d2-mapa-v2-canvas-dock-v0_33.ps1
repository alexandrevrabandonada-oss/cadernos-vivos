# CV — V2 Tijolo D2 — MapaV2 (canvas + dock + painel) + fix onHash — v0_33
...
$mapaCompPath = Join-Path $repo "src\components\v2\MapaV2.tsx"
...
EnsureDir (Split-Path -Parent $mapaCompPath)
EnsureDir (Split-Path -Parent $pagePath)

# -----------------------
# PATCH 1) Fix MapaDockV2: garantir onHash definido
# -----------------------
$rawDock = Get-Content -LiteralPath $dockPath -Raw
if ($rawDock -match 'addEventListener\("hashchange",\s*onHash\)') {
  # se não tiver "const onHash" perto, injeta no começo do effect
  if (-not ($rawDock -match 'const\s+onHash\s*=')) {
    $lines = $rawDock -split "`r?`n"
    ...
    foreach ($ln in $lines) {
      ...
      if ($ln -match 'useEffect\(\(\)\s*=>\s*\{') {
        $insideEffect = $true
        continue
      }

      if ($insideEffect -and (-not $inserted)) {
        # quando encontrarmos a primeira linha que mexe com hashchange/addEventListener, injeta antes
        if ($ln -match 'hashchange' -or $ln -match 'addEventListener') {
          # tenta preservar indentação
          $indent = ""
          if ($ln -match '^(\s*)') { $indent = $Matches[1] }
          $out.Insert($out.Count-1, ($indent + 'const onHash = () => setSelectedId(readHashId());')) | Out-Null
          $inserted = $true
          $changed = $true
        }
      }

      if ($insideEffect -and $ln -match '^\s*\}\s*,\s*\[\s*\]\s*\)\s*;') {
        $insideEffect = $false
      }
    }
    ...
    if ($changed) {
      ...
      Write-Host "[OK] patched: MapaDockV2.tsx (adicionou const onHash dentro do effect)"
      if ($bk) { Write-Host ("[BK] " + $bk) }
    } else {
      Write-Host "[SKIP] MapaDockV2: nada para patchar (não detectei ponto de inserção)."
    }
  } else {
    Write-Host "[OK] MapaDockV2: já tem const onHash."
  }
} else {
  Write-Host "[SKIP] MapaDockV2: não achei addEventListener(hashchange, onHash)."
}

# -----------------------
# PATCH 2) MapaV2 (canvas + dock + painel)
# -----------------------
$bk2 = $null
if (Test-Path -LiteralPath $mapaCompPath) { $bk2 = BackupFile $mapaCompPath }
...
WriteUtf8NoBom $mapaCompPath ($mapaLines -join "`n")
Write-Host ("[OK] wrote: " + $mapaCompPath)
if ($bk2) { Write-Host ("[BK] " + $bk2) }

# -----------------------
# PATCH 3) Page /v2/mapa (Next 16.1 params Promise)
# -----------------------
$bk3 = $null
if (Test-Path -LiteralPath $pagePath) { $bk3 = BackupFile $pagePath }
...
# REPORT
$rep = @()
$rep += "# CV — V2 Tijolo D2 v0_33 — MapaV2 (canvas + dock + painel)"
$rep += ""
$rep += "## O que entrou"
$rep += "- Fix MapaDockV2: garante const onHash antes de addEventListener(hashchange)."
$rep += "- MapaV2 client: normaliza mapa (array ou objeto com nodes/items) e renderiza canvas (lista clicavel) + painel."
$rep += "- Dock ao lado + selecao por hash sem mutar window.location via assignment."
$rep += "- Rota /c/[slug]/v2/mapa com params Promise (Next 16.1)."
$rep += ""
$rep += "## Arquivos"
$rep += "- src/components/v2/MapaDockV2.tsx"
$rep += "- src/components/v2/MapaV2.tsx"
$rep += "- src/app/c/[slug]/v2/mapa/page.tsx"
$rep += ""
$rep += "## Verify"
$rep += "- tools/cv-verify.ps1 (guard + lint + build)"
$rep += ""

WriteReport "cv-v2-tijolo-d2-mapa-v2-canvas-dock-v0_33.md" ($rep -join "`n") | Out-Null
Write-Host "[OK] D2 v0_33 aplicado e verificado."