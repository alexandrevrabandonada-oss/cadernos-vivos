...
# paths
$target = Join-Path $root 'src\app\c\[slug]\v2\linha-do-tempo\page.tsx'
...
$ts = Get-Date -Format 'yyyyMMdd-HHmmss'
$reportPath = Join-Path $reportsDir ('cv-step-b4c-refactor-v2-linha-do-tempo-motor-' + $ts + '.md')
...
$rep.Add('# CV — Step B4c: Refactor V2 Linha-do-tempo to use safe motor')
$rep.Add('')
$rep.Add('- when: ' + $ts)
...
$rep.Add('## NEXT')
if ($verifyExit -eq 0) {
  $rep.Add('- OK. Próximo: B4d (Mapa V2) ou B4e (Provas/Trilhas, se ainda usarem getCaderno).')
} else {
  $rep.Add('- Verify falhou. Corrigir o erro apontado e re-rodar.')
}
