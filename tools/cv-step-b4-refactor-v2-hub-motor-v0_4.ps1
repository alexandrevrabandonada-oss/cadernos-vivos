...
# 2) imports necessários (sem duplicar)
...
# REPORT (sem backticks, sem ``` )
$rep = New-Object System.Collections.Generic.List[string]
$rep.Add('# CV — Step B4: Refactor V2 Hub to use safe motor')
$rep.Add('')
$rep.Add('- when: ' + $ts)
...
$rep.Add('## NEXT')
if ($verifyExit -eq 0) {
  $rep.Add('- OK. Próximo: B4a (Debate V2), B4b (Linha), B4c (Mapa) — 1 página por tijolo.')
} else {
