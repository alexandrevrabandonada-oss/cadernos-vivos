...
$comp = @(
  ...
  '}',
  '',
  'function normId(v: any, idx: number): string {',
  ...
  '  const t = item.tags || item.tag || item.temas || item.labels;',
  ...
  '}',
  '',
  'export function ProvasV2(props: { slug: string; title: string; items: ProvaItem[] }) {',
  ...
  '    return raw.map((it: any, idx) => ({ ...it, id: normId(it && it.id, idx) })) as ProvaItem[];',
  '  }, [props.items]);',
  '',
  '  const tags = useMemo(() => {',
  ...
  '            type="button"',
  '            onClick={() => setTag("")}',
  '            style={{ cursor: "pointer", fontSize: 12, fontWeight: 800, padding: "6px 10px", borderRadius: 999, background: !tag ? "var(--accent)" : "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: !tag ? "#111" : "inherit" }}',
  ...
  '                type="button"',
  '                onClick={() => setTag(on ? "" : t)}',
  '                style={{ cursor: "pointer", fontSize: 12, fontWeight: 800, padding: "6px 10px", borderRadius: 999, background: on ? "var(--accent)" : "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: on ? "#111" : "inherit" }}',
  ...
  '              key={it.id}',
  '              id={it.id}',
  '              style={{ borderRadius: 14, padding: 12, border: isActive ? "2px solid var(--accent)" : "1px solid rgba(255,255,255,0.10)", background: "rgba(0,0,0,0.18)" }}',
  '            >',
  ...
  '                      style={{ cursor: "pointer", fontSize: 12, fontWeight: 900, padding: "6px 10px", borderRadius: 999, background: isActive ? "var(--accent)" : "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: isActive ? "#111" : "inherit" }}',
  '                    >',
  '                      #{it.id}',
  '                    </button>',
  ...
  '                    onClick={async () => { const ok = await copyText(ref); if (!ok) alert("Não consegui copiar aqui. Tenta manualmente."); }}',
  '                  >',
  '                    Copiar ref',
  '                  </button>',
  '                  <button',
  '                    type="button"',
  '                    title="Copiar link direto (com hash)"',
  '                    style={{ cursor: "pointer", fontSize: 12, fontWeight: 800, padding: "8px 10px", borderRadius: 10, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", color: "inherit" }}',
  '                    onClick={async () => {',
  '                      try {',
  '                        const base = window.location.href.split("#")[0];',
  '                        const link = base + "#" + it.id;',
  '                        const ok = await copyText(link);',
  '                        if (!ok) alert("Não consegui copiar aqui. Tenta manualmente.");',
  '                      } catch {',
  '                        alert("Não consegui gerar o link aqui.");',
  '                      }',
  '                    }}',
  '                  >',
  '                    Copiar link',
  ...
  '                      target="_blank"',
  '                      rel="noreferrer"',
  '                      style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 900, padding: "8px 10px", borderRadius: 10, background: "var(--accent)", color: "#111", textDecoration: "none" }}',
  '                      title="Abrir fonte"',
  '                    >',
  '                      Abrir',
  '                    </a>',
...
$rep.Add("# CV — Step D4 — Provas/Acervo V2 interativo (v0_57)") | Out-Null
$rep.Add("") | Out-Null
$rep.Add("## O que entrou") | Out-Null
$rep.Add("- ProvasV2 (client): busca, filtro por tags, foco por hash + scroll suave, copiar referência e copiar link direto.") | Out-Null
$rep.Add("- /c/[slug]/v2/provas: carrega caderno + acervo, aplica --accent e usa V2Nav.") | Out-Null
$rep.Add("") | Out-Null
...
$rp = WriteReport "cv-step-d4-provasv2-interactive-v0_57.md" ($rep -join "`n")
Write-Host ("[OK] Report: " + $rp)
Write-Host "[OK] Step D4 aplicado e verificado."