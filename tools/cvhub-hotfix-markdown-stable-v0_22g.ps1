...
WriteUtf8NoBom $mdPath $tsLines
WL "[OK] wrote: src/lib/markdown.ts (stable renderer + exports)"
...
$rep = @(
("# CV Hotfix — markdown.ts stable v0_22g — " + $now),
"",
"## O que foi feito",
"- Reescreveu src/lib/markdown.ts com renderer simples e estavel (sem escapes quebrados).",
"- Corrigiu strings de link (sem \\\\\"), e regex com \\d e \\s corretos.",
"- Exporta markdownToHtml e simpleMarkdownToHtml (alias para compat).",
"",
"## Verify",
"- npm run lint",
"- npm run build"
...
$repPath = NewReport "cv-hotfix-markdown-stable-v0_22g.md" $rep
...
WL ""
WL "[OK] Hotfix markdown aplicado (v0_22g)."