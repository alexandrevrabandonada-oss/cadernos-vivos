...
$stamp = (Get-Date -Format "yyyyMMdd-HHmmss")
$step  = "cv-step-b6c-v2-provas-filter-toolbar-copy-v0_2"
...
Write-Host ("[BK] " + (Join-Path "tools\_patch_backup" (Split-Path -Leaf $bkComp)))
...
if ($cssRaw -notmatch "cv2-domfilter") {
  ...
  Write-Host ("[BK] " + (Join-Path "tools\_patch_backup" (Split-Path -Leaf $bkCss)))
} else {
...
$rep = @()
$rep += "# CV — Step B6c: Provas V2 — filter toolbar + copy (v0_2)"
$rep += ""
$rep += "- when: " + $stamp
$rep += "- file: " + $compRel
...
$rep += ""
$rep += "## ACTIONS"
$rep += "- Rewrote Cv2DomFilterClient as stable named export using rootId."
$rep += "- Added buttons: Limpar / Copiar links / Copiar MD (only visible items)."
$rep += "- Optional CSS appended if missing."
$rep += ""
$rep += "## VERIFY"
$rep += "- tools/cv-verify.ps1"
WriteUtf8NoBomSafe $reportPath ($rep -join "`n")
Write-Host ("[REPORT] " + (Join-Path "reports" (Split-Path -Leaf $reportPath)))
...
Write-Host "[OK] B6c v0_2 aplicado." -ForegroundColor Green
