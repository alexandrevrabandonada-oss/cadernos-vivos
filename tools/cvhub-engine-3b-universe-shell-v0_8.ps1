...
# -------------------------
# PATCH 1 — criar layout por caderno (Universe Shell)
# -------------------------
EnsureDir $layoutDir
...
'  // Cada página ainda pode ter seu "modo", mas o pano de fundo é comum.',
...
# -------------------------
# PATCH 2 — adicionar CSS do universo (safe append)
# -------------------------
...
if ($raw -like "*/* cv-universe */*") {
  ...
  $block = @(
    ...
    ".cv-universe {",
    "  min-height: 100%;",
    "  /* atmosfera: camadas suaves (não depende do caderno ainda) */",
    "  background-image:",
...
$reportPath = Join-Path $repDir "cv-engine-3b-universe-shell-v0_8.md"
$report = @(
("# CV Engine-3B — Universe Shell v0.8 — " + $now),
"",
"## O que mudou",
...
"- Preparar o terreno para o próximo passo: 'cada página um universo' (moods por rota).",
"",
"## Verify",
"- npm run lint",
"- npm run build"
