...
# root (com fallback caso alguém cole e rode “na unha”)
...
$stamp = (Get-Date).ToString("yyyyMMdd-HHmmss")
$step = "cv-step-b6g-v2-provas-grouped-domainchips-dedupe-v0_1"
...
# -----------------------
# PATCH 1) rewrite component (parser-safe + chips + dedupe)
# -----------------------
...
$lines = @(
  ...
  '  let snap: Snapshot = { items: [], open: {}, activeDomain: null, dedupe: true, lastScan: 0, error: null };',
  ...
  '          md.push("");',
  '          if (snap.activeDomain) { md.push("> filtro: " + snap.activeDomain); md.push(""); }',
...
# -----------------------
# PATCH 2) globals.css additive: chip active state
# -----------------------
$cssRaw = Get-Content -Raw -LiteralPath $css
$marker = "/* CV2 Provas grouped chips */"
if ($cssRaw -notmatch [regex]::Escape($marker)) {
  $bk2 = BackupFileLocal $root $css
  $add = @(
    "",
    $marker,
    ".cv2-chip.is-active { outline: 2px solid rgba(255,255,255,0.18); }"
  ) -join "`n"
...
$rep = @()
$rep += "# CV — B6g: Provas V2 (Por domínio) — chips + dedupe + copiar MD"
$rep += ""
$rep += ("- when: " + $stamp)
...
$rep += ""
$rep += "## O que muda"
$rep += "- Chips de domínio (filtro do painel)."
$rep += "- Toggle Deduplicar (por href)."
$rep += "- Copiar (MD) respeita filtro do painel."
$rep += ""
$rep += "## VERIFY"
