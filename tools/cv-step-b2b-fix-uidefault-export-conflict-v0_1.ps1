...
$ts = Get-Date -Format 'yyyyMMdd-HHmmss'
$reportPath = Join-Path $reportsDir ('cv-step-b2b-fix-uidefault-export-conflict-' + $ts + '.md')
...
# Reescreve contract.ts SEM exportar UiDefault (UiDefault vem de ./types)
$lines = @(
  'import { z } from "zod";',
...
# REPORT
$rep = @()
$rep += '# CV — Step B2b: Fix UiDefault export conflict'
$rep += ''
$rep += ('- when: ' + $ts)
...
$rep += '## NEXT'
if ($verifyExit -eq 0) {
  $rep += '- ✅ Tudo verde. Próximo tijolo: B3 (integrar parse/normalize com fallbacks reais no V2).'
} else {
