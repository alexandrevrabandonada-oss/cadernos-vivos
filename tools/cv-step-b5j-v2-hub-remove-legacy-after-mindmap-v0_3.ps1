...
$stamp = _NowTag
Write-Host ("== cv-step-b5j-v2-hub-remove-legacy-after-mindmap-v0_3 == " + $stamp)
...
# procurar um "sinal" do bloco duplicado DEPOIS do mindmap
$needles = @(
...
if ($hitPos -lt 0) {
  Write-Host "[SKIP] Não achei sinal do bloco duplicado após o mindmap. Nada a remover."
} else {
  Write-Host ("[DIAG] Hit needle: '" + $hitNeedle + "' @ pos " + $hitPos)

  $toks = Get-Tokens $raw
  $containerTags = @("div","section","main","article")
  $forbid = @("Cv2MindmapHubClient","CV2_MINDMAP_HUB","Cv2CoreNodes","CV2_CORE_NODES","Cv2MapRail","Cv2MindmapHub")

  $best = Find-BestContainerSpan -raw $raw -toks $toks -hitPos $hitPos -needle $hitNeedle -containerTags $containerTags -forbid $forbid
  if (-not $best) {
    throw "[STOP] Achei o duplicado, mas não consegui isolar um container seguro (div/section/main/article) sem pegar Core/Mindmap."
  }

  Write-Host ("[PATCH] Removendo bloco duplicado <" + $best.Tag + "> span [" + $best.Open + ".." + $best.Close + ") len=" + $best.Len)

  $before = $raw.Substring(0, $best.Open)
  $after  = $raw.Substring($best.Close)
  $marker = "`n      {/* CV2_DUP_BLOCK_REMOVED " + $stamp + " needle=" + $hitNeedle + " */}`n"
  $newRaw = $before + $marker + $after

  WriteUtf8NoBom $hubAbs $newRaw
  Write-Host ("[OK] duplicado removido do Hub V2: " + $hubRel)
}
...
EnsureDir $repDir
$rep = Join-Path $repDir ($stamp + "-cv-step-b5j-v2-hub-remove-legacy-after-mindmap-v0_3.md")
$body = @(
  ("# CV B5J v0_3 — Hub V2: remover bloco duplicado após mindmap — " + $stamp),
  "",
  ("Repo: " + $repoRoot),
  "",
  "## PATCH",
...
Write-Host "[OK] B5J v0_3 concluído."