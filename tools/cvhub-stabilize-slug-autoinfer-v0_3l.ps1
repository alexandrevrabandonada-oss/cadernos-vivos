...
WL ("[DIAG] Repo: " + $repo)
WL ("[DIAG] npm: " + $npmExe)
WL ("[DIAG] NavPills: " + ($navPath ? $navPath : "(não achei)"))
WL ("[DIAG] AulaProgress: " + ($aulaProgPath ? $aulaProgPath : "(não achei)"))
WL ("[DIAG] DebateBoard: " + ($debPath ? $debPath : "(não achei)"))
WL ("[DIAG] TerritoryMap: " + ($mapPath ? $mapPath : "(não achei)"))

if (-not $navPath) { throw "[STOP] Não achei NavPills.tsx." }
if (-not $aulaProgPath) { throw "[STOP] Não achei AulaProgress.tsx." }
if (-not $debPath) { throw "[STOP] Não achei DebateBoard.tsx." }
if (-not $mapPath) { throw "[STOP] Não achei TerritoryMap.tsx." }
...
# NavPills (slug opcional, autoinfer SEM chamar hook dentro de helper)
$nav = @(
  '"use client";',
  '',
  ...
  '',
  'function pickSlug(propSlug: string | undefined, params: P): string {',
...
WL "[OK] NavPills.tsx atualizado."

# AulaProgress (slug opcional, autoinfer)
$aula = @(
  '"use client";',
  '',
...
WriteUtf8NoBom $aulaProgPath $aula
WL "[OK] AulaProgress.tsx atualizado."

# DebateBoard (slug opcional, autoinfer)
$deb = @(
  '"use client";',
  '',
...
WL "[OK] DebateBoard.tsx atualizado."

# TerritoryMap (slug opcional, autoinfer)
$map = @(
  '"use client";',
  '',
  'import { useMemo, useState } from "react";',
  ...
  '  const view = useMemo(() => (!filter ? points : points.filter((p) => (p.kind || "") === filter)), [points, filter]);',
  '',
  '  return (',
  '    <div className="card p-5">',
  ...
  '                  )}',
  '                </div>',
  '                {link ? <a className="text-sm accent hover:underline" href={link} target="_blank" rel="noreferrer">Abrir</a> : null}',
  '              </div>',
  '            </div>',
  '          );',
  '        })}',
...
WriteUtf8NoBom $mapPath $map
WL "[OK] TerritoryMap.tsx atualizado."

# -------------------------
# REPORT (sem backticks)
# -------------------------
$repDir = Join-Path $repo "reports"
EnsureDir $repDir
$now = Get-Date -Format "yyyy-MM-dd HH:mm"
$reportPath = Join-Path $repDir "cv-3l-stabilize-slug-autoinfer.md"

$lines = @(
  "# CV-3l — Stabilize slug autoinfer — " + $now,
  "",
  "## O que mudou",
  "- NavPills / AulaProgress / DebateBoard / TerritoryMap: slug virou opcional e é inferido via useParams quando não é passado.",
  "- Evita o whack-a-mole do TypeScript: Property 'slug' is missing...",
  "",
  "## Verify",
  "- npm run lint",
  "- npm run build"
...
WriteUtf8NoBom $reportPath $lines
...
WL "[OK] Aplicado. Agora pode chamar esses componentes sem passar slug."