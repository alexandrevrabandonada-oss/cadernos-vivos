...
# paths
$target = Join-Path $root 'src\app\c\[slug]\v2\mapa\page.tsx'
...
$ts = Get-Date -Format 'yyyyMMdd-HHmmss'
$reportPath = Join-Path $reportsDir ('cv-step-b4d-refactor-v2-mapa-motor-' + $ts + '.md')
...
$rep.Add('# CV — Step B4d: Refactor V2 Mapa to use safe motor')
$rep.Add('')
$rep.Add('- when: ' + $ts)
...
$rep.Add('## NEXT')
if ($verifyExit -eq 0) {
  $rep.Add('- OK. Próximo: B4e (Provas V2) e B4f (Trilhas V2), se ainda tiverem getCaderno.')
} else {
