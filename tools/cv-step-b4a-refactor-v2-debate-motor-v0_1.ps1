...
# paths
$target = Join-Path $root 'src\app\c\[slug]\v2\debate\page.tsx'
...
$ts = Get-Date -Format 'yyyyMMdd-HHmmss'
$reportPath = Join-Path $reportsDir ('cv-step-b4a-refactor-v2-debate-motor-' + $ts + '.md')
...
$rep.Add('# CV — Step B4a: Refactor V2 Debate to use safe motor')
$rep.Add('')
$rep.Add('- when: ' + $ts)
...
$rep.Add('## NEXT')
if ($verifyExit -eq 0) {
  $rep.Add('- OK. Próximo: B4b (Linha V2).')
} else {
