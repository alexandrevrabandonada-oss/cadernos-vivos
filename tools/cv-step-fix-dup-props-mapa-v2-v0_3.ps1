...
function ParseProps([string]$tag) {
  ...
  while ($i -lt $tag.Length) {
    ...
    if ($j -lt $tag.Length -and $tag[$j] -eq '=') {
      ...
      $props += @{ name=$name; segStart=$segStartWs; segEnd=$segEnd; raw=$tag.Substring($segStartWs, $segEnd-$segStartWs) }
      $i = $j
    } else {
      # boolean prop
      $segEnd2 = $i
      $segStartWs2 = ExpandLeftWs $tag $segStart
      $props += @{ name=$name; segStart=$segStartWs2; segEnd=$segEnd2; raw=$tag.Substring($segStartWs2, $segEnd2-$segStartWs2) }
      $i = $j
    }
  }
...
function FixTagRemoveEarlierDuplicates([string]$tag, [ref]$dupSummary) {
  ...
  foreach ($k in $dupes) {
    ...
    $dupSummary.Value += @(@{ prop=$k; count=$items.Count })
...
foreach ($t in $tags) {
  $dups = @()
  $newTag = FixTagRemoveEarlierDuplicates $t.text ([ref]$dups)
  if ($newTag -ne $t.text) {
    $fixed++
    $repls += @{ start=$t.start; end=$t.end; new=$newTag; old=$t.text; dups=$dups }
