# CV — V2 Hotfix — remover no-explicit-any (HomeV2 + /v2 page) — v0_26
...
$home = Join-Path $repo "src\components\v2\HomeV2.tsx"
$page = Join-Path $repo "src\app\c\[slug]\v2\page.tsx"

if (-not (Test-Path -LiteralPath (Split-Path -Parent $home))) { EnsureDir (Split-Path -Parent $home) }
if (-not (Test-Path -LiteralPath (Split-Path -Parent $page))) { EnsureDir (Split-Path -Parent $page) }
...
WriteUtf8NoBom $home ($homeLines -join "`n")
Write-Host ("[OK] wrote: " + $home)
...
# 4) REPORT
$report = @(
"# CV — Hotfix v0_26 — remove no-explicit-any (HomeV2 + /v2)",
"",
"## Causa raiz",
"- ESLint @typescript-eslint/no-explicit-any não permite (data as any) / (pano as any).",
"",
"## Fix",
"- HomeV2: usa Record<string, unknown> + acesso seguro pano['hot'] (sem any).",
"- /v2 page: helpers asObj/asStr e extração segura de meta.title/panorama (sem any).",
"",
"## Arquivos",
...
WriteReport "cv-v2-hotfix-no-explicit-any-homev2-page-v0_26.md" $report | Out-Null
Write-Host "[OK] v0_26 aplicado e verificado."