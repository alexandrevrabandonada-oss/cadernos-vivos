...
# PATHS
# -----------------------
$cardPath = Join-Path $repo "src\components\v2\V2HomeCard.tsx"
$pagePath = Join-Path $repo "src\app\c\[slug]\v2\page.tsx"

Write-Host ("[DIAG] Card: " + $cardPath)
Write-Host ("[DIAG] Page: " + $pagePath)

EnsureDir (Split-Path -Parent $cardPath)
EnsureDir (Split-Path -Parent $pagePath)

# -----------------------
# 1) COMPONENT: V2HomeCard (fora do render)
# -----------------------
$bk1 = BackupFile $cardPath
...
WriteUtf8NoBom $cardPath ($cardLines -join "`n")
Write-Host ("[OK] wrote: " + $cardPath)
if ($bk1) { Write-Host ("[BK] " + $bk1) }

# -----------------------
# 2) PAGE: Home V2 (Concreto Zen + Portas + Fios Quentes)
# -----------------------
if (-not (Test-Path -LiteralPath $pagePath)) {
  throw ("[STOP] nao achei page.tsx: " + $pagePath)
}
...
# -----------------------
$rep = @()
$rep += "# CV — V2 Tijolo D1 v0_42 — Home V2 Concreto Zen"
$rep += ""
$rep += "## O que entrou"
$rep += "- Home V2 com visual Concreto Zen (header + cards + seções)."
$rep += "- Portas: Mapa, Debate, Provas, Linha, Linha do tempo, Trilhas."
$rep += "- Fios quentes: nós mais conectados do mapa (heurística por grau), linkando para /v2/mapa#id."
$rep += "- Sem componente criado dentro do render (lint)."
$rep += "- Sem any; parsing resiliente para mapa/meta."
$rep += ""
$rep += "## Arquivos"
$rep += "- src/components/v2/V2HomeCard.tsx"
$rep += "- src/app/c/[slug]/v2/page.tsx"
$rep += ""
$rep += "## Verify"
$rep += "- tools/cv-verify.ps1 (guard + lint + build)"
$rep += ""

$rp = WriteReport "cv-v2-tijolo-d1-home-v2-concretozen-v0_42.md" ($rep -join "`n")
Write-Host ("[OK] Report: " + $rp)
Write-Host "[OK] D1 v0_42 aplicado."