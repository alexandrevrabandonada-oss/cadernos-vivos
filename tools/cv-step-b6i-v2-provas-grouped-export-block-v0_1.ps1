...
$stamp = (Get-Date).ToString("yyyyMMdd-HHmmss")
$step = "cv-step-b6i-v2-provas-grouped-export-block-v0_1"
...
# -----------------------
# PATCH 1) rewrite component (export bloco + toggle somente domínio ativo)
# -----------------------
...
$lines = @(
  ...
  '  let snap: Snapshot = { items: [], open: {}, activeDomain: null, dedupe: true, exportOnlyActive: true, lastScan: 0, error: null, toast: null };',
  ...
  '}',
  '',
  'function buildMd(ctx: { title: string; slug: string; url: string }, groups: Group[], activeDomain: string | null, label: string): string {',
  '  const md: string[] = [];',
  '  md.push("# Provas — " + ctx.title);',
  '  md.push("");',
  '  if (ctx.slug) md.push("> caderno: " + ctx.slug);',
  '  if (ctx.url) md.push("> link: " + ctx.url);',
  '  md.push("> export: " + label);',
  '  if (activeDomain) md.push("> domínio ativo: " + activeDomain);',
  '  md.push("");',
  ...
  '}',
  '',
  'function buildTxt(ctx: { title: string; slug: string; url: string }, items: LinkItem[], activeDomain: string | null, label: string): string {',
  '  const out: string[] = [];',
  '  out.push("Provas — " + ctx.title);',
  '  if (ctx.url) out.push(ctx.url);',
  '  out.push("export: " + label);',
  '  if (activeDomain) out.push("domínio ativo: " + activeDomain);',
  '  out.push("");',
  '  for (const it of items) out.push(it.href);',
  '  return out.join("\\n");',
  '}',
  '',
  ...
  '  }, [itemsBase]);',
  '',
  '  const groupsFiltered: Group[] = useMemo(() => {',
  '    const by: Record<string, LinkItem[]> = {};',
  ...
  '  }, [itemsFiltered]);',
  '',
  '  const visibleGroups = snap.activeDomain ? groupsFiltered : groupsAll;',
  '  const domainKeysVisible = useMemo(() => visibleGroups.map((g) => g.domain), [visibleGroups]);',
  '',
  '  function scanNow() {',
  '    try {',
  ...
  '',
  '  const exportLabel = snap.exportOnlyActive ? "somente domínio ativo (se houver)" : "tudo (ignora domínio ativo)";',
  '  const exportGroups = snap.exportOnlyActive ? groupsFiltered : groupsAll;',
  '  const exportItems  = snap.exportOnlyActive ? itemsFiltered : itemsBase;',
  '  const exportActive = snap.exportOnlyActive ? snap.activeDomain : null;',
  '',
  '  return (',
  ...
  '        <button type="button" className={"cv2-chip" + (snap.exportOnlyActive ? " is-active" : "")} onClick={() => store.set({ exportOnlyActive: !snap.exportOnlyActive })}>Somente domínio ativo</button>',
  ...
  '          const ctx = getCtx();',
  '          const md = buildMd(ctx, groupsFiltered, snap.activeDomain, "respeita domínio ativo");',
  '          const ok = copyText(md);',
  '          toast(ok ? "Copiado (MD)" : "Falha ao copiar", ok ? "ok" : "err");',
  '        }}>Copiar (MD)</button>',
  '        <button type="button" className="cv2-chip" onClick={() => {',
  '          const ctx = getCtx();',
  '          const txt = buildTxt(ctx, itemsFiltered, snap.activeDomain, "respeita domínio ativo");',
  '          const ok = copyText(txt);',
  '          toast(ok ? "Copiado (TXT)" : "Falha ao copiar", ok ? "ok" : "err");',
  '        }}>Copiar links (TXT)</button>',
  '        <button type="button" className="cv2-chip" onClick={() => {',
  '          const ctx = getCtx();',
  '          const md = buildMd(ctx, exportGroups, exportActive, exportLabel);',
  '          const txt = buildTxt(ctx, exportItems, exportActive, exportLabel);',
  '          const block: string[] = [];',
  '          block.push("===== PROVAS (MD) =====");',
  '          block.push(md);',
  '          block.push("");',
  '          block.push("===== LINKS (TXT) =====");',
  '          block.push(txt);',
  '          const ok = copyText(block.join("\\n"));',
  '          toast(ok ? "Exportado (bloco)" : "Falha ao exportar", ok ? "ok" : "err");',
  '        }}>Exportar bloco</button>',
  '      </div>',
  '',
...
$rep = @()
$rep += "# CV — B6i: Exportar bloco (MD+TXT) + toggle 'Somente domínio ativo'"
$rep += ""
$rep += ("- when: " + $stamp)
$rep += ("- file: " + $compRel)
$rep += ""
$rep += "## O que muda"
$rep += "- Botão: Exportar bloco (MD + TXT em um payload)."
$rep += "- Toggle: 'Somente domínio ativo' (quando OFF, exporta tudo)."
$rep += ""
$rep += "## VERIFY"
