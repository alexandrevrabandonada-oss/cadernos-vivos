...
$step  = "cv-step-b6u8-v2-map-axis-rail-v0_1"
...
if (-not (Test-Path $mapPageAbs)) { throw ("[STOP] não achei: " + $mapPageAbs) }
if (-not (Test-Path $globalsAbs)) { throw ("[STOP] não achei: " + $globalsAbs) }
...
Write-Host "[OK] B6U8 concluído (Mapa eixo + Rail de Portas)."