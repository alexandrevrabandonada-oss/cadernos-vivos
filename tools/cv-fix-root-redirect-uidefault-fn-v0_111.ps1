...
# -----------------------------------------
# Fix: trocar "if (uiDefault === 'v2')" por cálculo seguro do valor
# -----------------------------------------
PatchText "src\app\c\[slug]\page.tsx" {
  ...
  # pega o bloco inteiro desse if (bem simples: do 'if' até o próximo '}')
  $end = $out.IndexOf("}", $idx)
...
$rep.Add("# CV — Fix — root redirect uiDefault function-safe (v0_111)") | Out-Null
$rep.Add("") | Out-Null
$rep.Add("## O que foi corrigido") | Out-Null
$rep.Add("- /c/[slug]/page.tsx: uiDefault pode ser função; calculamos `ui` e comparamos com ""v2"" (corrige TypeScript).") | Out-Null
$rep.Add("") | Out-Null
...
$rp = WriteReport "cv-fix-root-redirect-uidefault-fn-v0_111.md" ($rep -join "`n")
