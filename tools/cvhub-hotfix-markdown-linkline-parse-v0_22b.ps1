...
# encontra e substitui a linha do replace de links
$lines = $raw -split "`r?`n"
...
if ($changed -eq 0) {
  throw "[STOP] Não achei a linha do replace de links no markdown.ts. Me manda as linhas em volta do trecho inline()."
}
...
# REPORT
$rep = @()
$rep += ("# Hotfix — markdown.ts link replace parse v0.22b — " + (Get-Date -Format "yyyy-MM-dd HH:mm"))
$rep += ""
...
$repPath = NewReport "cv-hotfix-markdown-linkline-parse-v0_22b.md" ($rep -join "`n")
