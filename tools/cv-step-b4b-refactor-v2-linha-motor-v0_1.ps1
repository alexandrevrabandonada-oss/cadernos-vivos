...
# paths
$target = Join-Path $root 'src\app\c\[slug]\v2\linha\page.tsx'
...
$ts = Get-Date -Format 'yyyyMMdd-HHmmss'
$reportPath = Join-Path $reportsDir ('cv-step-b4b-refactor-v2-linha-motor-' + $ts + '.md')
...
$rep.Add('# CV — Step B4b: Refactor V2 Linha to use safe motor')
$rep.Add('')
$rep.Add('- when: ' + $ts)
...
$rep.Add('## NEXT')
if ($verifyExit -eq 0) {
  $rep.Add('- OK. Próximo: B4c (Mapa V2) ou B4c (Linha-do-tempo V2).')
} else {
