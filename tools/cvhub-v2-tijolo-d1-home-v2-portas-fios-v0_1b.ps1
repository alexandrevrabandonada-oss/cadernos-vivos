# CV — V2 — Tijolo D1 — Home V2 (3 portas + fios quentes) — v0_1b
...
# ⚠️ não usar $home (colide com $HOME)
...
Write-Host ("[DIAG] home v2: " + $v2HomePath)
Write-Host ("[DIAG] mapa v2: " + $v2MapaPath)
Write-Host ("[DIAG] debate v2: " + $v2DebatePath)
Write-Host ("[DIAG] provas v2: " + $v2ProvasPath)

# -------------------------
# PATCH: /c/[slug]/v2 — Home V2 com portas + fios quentes
# -------------------------
EnsureDir (Split-Path -Parent $v2HomePath)
$bkHome = BackupFile $v2HomePath
...
'    title = (c as any)?.meta?.title || slug;',
'    subtitle = (c as any)?.meta?.subtitle ?? null;',
'    uiDefault = (c as any)?.meta?.ui?.default || "v2";',
...
'  try {',
'    const c = await loadCadernoV2(slug);',
'    cTitle = (c as any)?.meta?.title || slug;',
'  } catch {',
'    // ok',
'  }',
'  return (',
...
WritePlaceholderPage $v2MapaPath "Mapa V2" "Em breve: canvas + painel lateral + dock (Tijolo D2)."
WritePlaceholderPage $v2DebatePath "Debate V2" "Em breve: perguntas + posições + provas (Tijolo D3)."
WritePlaceholderPage $v2ProvasPath "Provas V2" "Em breve: galeria + filtros + preview (Tijolo D4)."
...
EnsureDir $reports
$reportPath = Join-Path $reports "cv-v2-tijolo-d1-home-v2-portas-fios-v0_1b.md"
$report = @(
...
Write-Host "[OK] Tijolo D1 (v0_1b) aplicado."