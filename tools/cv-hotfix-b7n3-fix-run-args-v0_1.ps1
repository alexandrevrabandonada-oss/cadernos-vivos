# CV HOTFIX B7N3 — Fix Run args binding (npm run lint/build)
...
Write-Host ("== CV HOTFIX B7N3 FIX RUN ARGS == " + $stamp)
...
$reportPath = Join-Path $reportsDir ($stamp + "-cv-hotfix-b7n3-fix-run-args.md")

$targetRel = "tools\cv-step-b7n-map-core-highlights-v0_2.ps1"
$targetAbs = Join-Path $root $targetRel

$log = New-Object System.Collections.Generic.List[string]
$log.Add("# CV HOTFIX B7N3 — Fix Run args binding — " + $stamp) | Out-Null
$log.Add("") | Out-Null
...
# 1) Fix Run signature
$patched = $patched -replace 'function\s+Run\(\$cmd,\s*\$args\)', 'function Run([string]$cmd, [string[]]$args)'

# 2) Fix calls Run $npm @("run","lint") -> Run $npm ("run","lint")
$patched = $patched -replace 'Run\s+\$npm\s+@\("run","lint"\)', 'Run $npm ("run","lint")'
$patched = $patched -replace 'Run\s+\$npm\s+@\("run","build"\)', 'Run $npm ("run","build")'

# 3) (safety) if someone used @("run","lint") with spaces
$patched = $patched -replace 'Run\s+\$npm\s+@\(\s*"run"\s*,\s*"lint"\s*\)', 'Run $npm ("run","lint")'
$patched = $patched -replace 'Run\s+\$npm\s+@\(\s*"run"\s*,\s*"build"\s*\)', 'Run $npm ("run","build")'
...
if ($patched -eq $raw) {
  ...
  $log.Add("[OK] patch aplicado.") | Out-Null
...
try {
  ...
  $log.Add("```") | Out-Null
  $log.Add("") | Out-Null
  $log.Add("[OK] B7N executado (ver log acima).") | Out-Null
} catch {
