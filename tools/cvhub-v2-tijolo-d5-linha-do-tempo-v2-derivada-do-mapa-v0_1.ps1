# CV — V2 — Tijolo D5: Linha do tempo V2 (derivada do mapa) — v0_1
...
$timelinePage = Join-Path $repo "src\app\c\[slug]\v2\linha\page.tsx"
$timelineComp = Join-Path $repo "src\components\v2\TimelineV2.tsx"

Write-Host ("[DIAG] page: " + $timelinePage)
Write-Host ("[DIAG] comp: " + $timelineComp)

# -------------------------
# PATCH 1: Client component TimelineV2
# -------------------------
EnsureDir (Split-Path -Parent $timelineComp)
$bkC = BackupFile $timelineComp
...
'          <div style={{ opacity: 0.65, fontSize: 12 }}>V2 • {props.title}</div>',
'        </div>',
'',
...
WriteUtf8NoBom $timelineComp $comp
Write-Host "[OK] wrote: TimelineV2.tsx"
if ($bkC) { Write-Host ("[BK] " + $bkC) }

# -------------------------
# PATCH 2: Page server /c/[slug]/v2/linha
# -------------------------
EnsureDir (Split-Path -Parent $timelinePage)
$bkP = BackupFile $timelinePage
...
'          <h1 style={{ fontSize: 26, fontWeight: 900, letterSpacing: "-0.03em", marginTop: 6 }}>Linha do tempo</h1>',
...
WriteUtf8NoBom $timelinePage $page
Write-Host "[OK] wrote: /c/[slug]/v2/linha (D5)"
...
EnsureDir $reports
$reportPath = Join-Path $reports "cv-v2-tijolo-d5-linha-do-tempo-v2-derivada-do-mapa-v0_1.md"

$report = @(
  "# CV — V2 — Tijolo D5 (Linha do tempo V2)",
  "",
  "## Entrega",
  "- Nova rota: /c/[slug]/v2/linha",
  "- Linha do tempo derivada do campo mapa (nodes) do caderno",
  "- Busca + filtros por tipo (kind/type) e tags",
  "- Ordenacao por date/day/when/ano (quando existir); sem data vai para o fim",
  "- Copiar link de cada item (ancora) e abrir fonte (url/link) quando existir",
  "",
  "## Arquivos",
...
Write-Host "[OK] Tijolo D5 aplicado."