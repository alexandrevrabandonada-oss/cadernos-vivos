...
function WriteFileLines([string]$rel, [string[]]$lines) {
  $full2 = Join-Path $repo $rel
  EnsureDir (Split-Path -Parent $full2) | Out-Null
  $bk = $null
  if (Test-Path -LiteralPath $full2) { $bk = BackupFile $full2 }
  $content = $lines -join "`r`n"
  WriteUtf8NoBom $full2 $content
  Write-Host ("[OK] wrote: " + $full2)
  if ($bk) { Write-Host ("[BK] " + $bk) }
  $script:changed.Add($full2) | Out-Null
}
...
'                {nodes.map((n) => {',
'                  const isActive = !!activeId && activeId === n.id;',
'                  const left = typeof n.x === "number" ? n.x : 40 + (Math.random() * 40);',
'                  const top = typeof n.y === "number" ? n.y : 40 + (Math.random() * 40);',
'                  return (',
'                    <button',
'                      key={n.id}',
'                      id={"node-" + n.id}',
...
WriteFileLines "src\components\v2\MapaV2.tsx" $mapCompLines

# 2) Page: /c/[slug]/v2/mapa
$pageLines = @(
...
WriteFileLines "src\app\c\[slug]\v2\mapa\page.tsx" $pageLines
...
$rep.Add("# CV — Step D2 — Mapa V2 (v0_72)") | Out-Null
$rep.Add("") | Out-Null
$rep.Add("## O que entrou") | Out-Null
$rep.Add("- MapaV2 (client): painel com busca, foco por hash, canvas (grid ou x/y), dock com ações (copiar link, id, json).") | Out-Null
$rep.Add("- /c/[slug]/v2/mapa: carrega caderno, aplica --accent e injeta mapa do conteúdo.") | Out-Null
$rep.Add("") | Out-Null
...
$rp = WriteReport "cv-step-d2-mapav2-canvas-panel-dock-v0_72.md" ($rep -join "`n")
Write-Host ("[OK] Report: " + $rp)
Write-Host "[OK] Step D2 aplicado e verificado."