...
# ---- DebateBoard.tsx (slug opcional + autoinfer) ----
BackupFile $debPath
$debLines = @(
'"use client";',
'',
...
WL "[OK] wrote: DebateBoard.tsx (slug opcional/autoinfer)"

# ---- AulaProgress.tsx (slug opcional + autoinfer) ----
BackupFile $apPath
$apLines = @(
'"use client";',
'',
...
'        </div>',
'        <div className="text-sm muted">{pct}%{flash === "ok" ? " • salvo" : ""}</div>',
'      </div>',
...
WL "[OK] wrote: AulaProgress.tsx (slug opcional/autoinfer)"

# ---- TerritoryMap.tsx (sem any + slug opcional/autoinfer) ----
BackupFile $mapPath
$mapLines = @(
'"use client";',
'',
...
WL "[OK] wrote: TerritoryMap.tsx (sem any + slug opcional/autoinfer)"

# ---- RegistroPanel.tsx (novo) ----
...
'          Isso é o “recibo” do Cadernos Vivos — não tem nada a ver com o app ECO.',
'        </p>',
'      </div>',
'',
'      <div className="card p-5">',
...
WL "[OK] wrote: RegistroPanel.tsx (novo)"
...
$reportPath = Join-Path $repDir "cv-4-registro-slug-optional-v0_4.md"

$reportText = @"
# CV-4 — Registro do Caderno + slug opcional (v0.4) — $now

## Mudança de estratégia
- Componentes client **não exigem mais** \`slug\` (agora é opcional e é inferido via \`useParams()\`).
...
- Reescrito: \`src/components/AulaProgress.tsx\` (slug opcional/autoinfer + salva progresso)
