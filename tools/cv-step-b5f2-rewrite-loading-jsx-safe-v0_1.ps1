...
function BackupFile([string]$filePath, [string]$backupDir, [string]$suffix) {
  EnsureDir $backupDir
  $ts = Get-Date -Format "yyyyMMdd-HHmmss"
  $name = (Split-Path -Leaf $filePath) -replace "\.tsx$",".tsx"
  $safeSuffix = ($suffix -replace "[^a-zA-Z0-9_\-]","_")
  $dest = Join-Path $backupDir ($ts + "-" + $safeSuffix + "-" + $name + ".bak")
...
function CanonLoadingJsx([string]$title, [int]$count, [string]$mode) {
  # modo sempre "hub" ou "list"
  $m = $mode.Trim().Replace("`r","").Replace("`n","")
  $t = $title.Trim().Replace("`r","").Replace("`n","")
...
$ts = Get-Date -Format "yyyyMMdd-HHmmss"
$step = "cv-step-b5f2-rewrite-loading-jsx-safe"
...
$targets = @(
  @{ rel = 'src\app\c\[slug]\v2\loading.tsx';              title = 'Carregando hub…';    count = 6; mode = 'hub'  },
  @{ rel = 'src\app\c\[slug]\v2\trilhas\loading.tsx';      title = 'Carregando trilhas…'; count = 6; mode = 'list' },
  @{ rel = 'src\app\c\[slug]\v2\trilhas\[id]\loading.tsx'; title = 'Carregando trilha…';  count = 5; mode = 'list' },
  @{ rel = 'src\app\c\[slug]\v2\provas\loading.tsx';       title = 'Carregando provas…';  count = 6; mode = 'list' }
)

foreach ($t in $targets) {
  $p = Join-Path $root $t.rel
  if (-not (Test-Path -LiteralPath $p)) {
    $actions.Add("SKIP (not found): " + $t.rel)
    continue
  }

  $raw = ReadText $p
  $canon = CanonLoadingJsx $t.title ([int]$t.count) ([string]$t.mode)

  if ($raw -ne $canon) {
    $bk = BackupFile $p $backupDir ($t.rel -replace "\\","_")
    $backups.Add((Split-Path -Leaf $bk))
    WriteUtf8NoBom $p $canon
    $actions.Add("Rewrote JSX-safe loading.tsx: " + $t.rel)
  } else {
    $actions.Add("OK already JSX-safe: " + $t.rel)
  }
}
...
$rep.Add("# CV — Step B5f2: rewrite loading.tsx JSX-safe (no multiline string props)")
$rep.Add("")
$rep.Add("- when: " + $ts)
$rep.Add("- repo: " + $root)
$rep.Add("")
