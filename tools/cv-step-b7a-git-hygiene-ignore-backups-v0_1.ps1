# cv-step-b7a-git-hygiene-ignore-backups-v0_1
...
$r.AddRange((TryRun "Git status" { git status })) | Out-Null
$r.AddRange((TryRun "Git diff --stat" { git diff --stat })) | Out-Null
...
$r.AddRange((TryRun "cv-verify.ps1 (se existir)" {
...
$r.AddRange((TryRun "npm run lint" {
  $npm = (Get-Command npm.cmd -ErrorAction Stop).Path
  & $npm run lint
})) | Out-Null

$r.AddRange((TryRun "npm run build" {
  $npm = (Get-Command npm.cmd -ErrorAction Stop).Path
  & $npm run build
})) | Out-Null
...
Write-Host "[OK] Tijolo B7A finalizado."