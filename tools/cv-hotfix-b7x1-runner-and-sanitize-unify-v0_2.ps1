# tools/cv-hotfix-b7x1-runner-and-sanitize-unify-v0_2.ps1
# Fixes:
# 1) sanitize script: .Replace($c,"") -> .Replace([string]$c,"")
# 2) runner: RunCmd/RunNpm robustos, sempre com npm.cmd e args corretos
...
function ReplaceFunctionBlock([string]$text, [string]$funcName, [string]$newFuncText) {
  $rx = "(?ms)^\s*function\s+$([regex]::Escape($funcName))\b"
...
function InsertAfterFunction([string]$text, [string]$afterFuncName, [string]$insertText) {
  $rx = "(?ms)^\s*function\s+$([regex]::Escape($afterFuncName))\b"
  $m = [regex]::Match($text, $rx)
...
AddLine $r "# CV HOTFIX B7X1 v0_2 — Runner robusto + sanitize fix"
AddLine $r ""
