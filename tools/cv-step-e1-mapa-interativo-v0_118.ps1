...
WriteRel "src\components\v2\MapaV2Client.tsx" $mapaClient

# ---------------------------
# 2) Server wrapper: MapaV2Interactive.tsx
# ---------------------------
$mapaServer = @(
  ...
  '}',
  '',
  'export default async function MapaV2Interactive(props: { slug: string; title?: string }) {',
...
WriteRel "src\components\v2\MapaV2Interactive.tsx" $mapaServer

# ---------------------------
# 3) Garantir /v2/mapa/page.tsx (usar MapaV2Interactive)
# ---------------------------
$mapaPageRel = "src\app\c\[slug]\v2\mapa\page.tsx"
$mapaPageAbs = Join-Path $repo $mapaPageRel

if (Test-Path -LiteralPath $mapaPageAbs) {
  PatchText $mapaPageRel {
    param($s)
    $out = $s
    if ($out.IndexOf('MapaV2Interactive') -ge 0) { return $out }

    # troca import se existir MapaV2
    if ($out.IndexOf('from "@/components/v2/MapaV2"') -ge 0) {
      $out = $out.Replace('from "@/components/v2/MapaV2"', 'from "@/components/v2/MapaV2Interactive"')
      $out = $out.Replace('MapaV2', 'MapaV2Interactive')
      return $out
    }

    # se não achar, injeta um import e troca componente no JSX se possível
    if ($out.IndexOf('import') -ge 0 -and $out.IndexOf('getCaderno') -ge 0) {
      $out = $out.Replace('import V2Nav from "@/components/v2/V2Nav";', 'import V2Nav from "@/components/v2/V2Nav";' + "`r`n" + 'import MapaV2Interactive from "@/components/v2/MapaV2Interactive";')
      $out = $out.Replace('<MapaV2 ', '<MapaV2Interactive ')
      $out = $out.Replace('</MapaV2>', '</MapaV2Interactive>')
      return $out
    }
  ...
  $mapaPage = @(
    ...
    '      <V2Nav slug={slug} active={"mapa" as any} />',
    '      <MapaV2Interactive slug={slug} title={title} />',
    '    </main>',
    '  );',
    '}'
  )
  WriteRel $mapaPageRel $mapaPage
}
...
$rep.Add("# CV — Step E1 — Mapa V2 interativo (v0_118)") | Out-Null
$rep.Add("") | Out-Null
$rep.Add("## O que entrou") | Out-Null
$rep.Add("- Novo client component: MapaV2Client (busca, seleção, vizinhos via edges).") | Out-Null
$rep.Add("- Novo wrapper server-safe: MapaV2Interactive (lê mapa.json e/ou mapa.md).") | Out-Null
$rep.Add("- /c/[slug]/v2/mapa agora usa MapaV2Interactive (cria ou patch).") | Out-Null
$rep.Add("") | Out-Null
...
$rp = WriteReport "cv-step-e1-mapa-interativo-v0_118.md" ($rep -join "`n")
Write-Host ("[OK] Report: " + $rp)
Write-Host "[OK] E1 aplicado e verificado."