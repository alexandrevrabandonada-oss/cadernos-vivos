...
# procura meta.json fora de node_modules/.next/tools/_patch_backup/reports
$metas = Get-ChildItem -LiteralPath $repo -Recurse -File -Filter "meta.json" -ErrorAction SilentlyContinue |
  Where-Object {
    $_.FullName -notmatch "\\node_modules\\|\\\.next\\|\\tools\\_patch_backup\\|\\reports\\"
  }

if (!$metas -or $metas.Count -eq 0) {
...
foreach ($m in $metas) {
  try {
    ...
    # heurística: meta de caderno costuma ter title/accent
    $title = $null
...
Write-Host ""
Write-Host "[NEXT] use um slug real acima e abre:"
