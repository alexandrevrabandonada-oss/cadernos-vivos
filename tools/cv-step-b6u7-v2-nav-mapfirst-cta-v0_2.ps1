...
Write-Host ("== cv-step-b6u7-v2-nav-mapfirst-cta-v0_2 == " + $stamp)
...
# ------------------------------------------------------------
$candidates = @(
  Get-ChildItem -LiteralPath (Join-Path $repoRoot "src") -Recurse -File -Filter "V2Nav.tsx" -ErrorAction SilentlyContinue
)

if (-not $candidates -or $candidates.Count -eq 0) {
  throw "[STOP] não achei V2Nav.tsx em src/**"
}

$prefer = ($candidates | Where-Object { $_.FullName -match "\\src\\components\\v2\\V2Nav\.tsx$" } | Select-Object -First 1)
if ($prefer) { $navAbs = $prefer.FullName } else { $navAbs = $candidates[0].FullName }
